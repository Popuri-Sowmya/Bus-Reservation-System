...
import { faSquare,faAngleDoubleDown } from '@fortawesome/free-solid-svg-icons';
import '../styles/Tab.css'
import { useDispatch } from 'react-redux';
import { updateSelectedSeats } from './actions/BusActions';
import { useNavigate } from "react-router-dom";
import { updatePassengerNames } from './actions/seatSelectionActions';

export default function SeatSelection() {
    const dispatch = useDispatch();
    const [name, setName] = useState([])
    const [gender, setGender] = useState([])
    const [reservedSeat, setReservedSeat] = useState(["1A", "2A", "3B", "4A", "5C", "7B", "7C", '8B', "9B"])
    ...
    const handleSubmitDetails = e => {
        ...
        if(seatNumber.length == 0){
            window.alert("No seats selected!! Select seats to continue!")
            navigate("/display")
            return;
        }
        dispatch(updateSelectedSeats(seatNumber));
        dispatch(updatePassengerNames(name));
        window.alert('No.of Seats Confirmed: '+seatNumber.length)
        navigate("/payment")
    }
    ...
    return (
        ...
        <ul className="nav nav-pills">
            <li className="nav-item">
                <a className="nav-link active" data-toggle="pill" href="#">Select Seat</a>
            </li>
        </ul>
    </div>
            <div className="ss">
                                ...
                                <ol className="cabin">
                                    <li className="row row--1">
                                        <ol className="seats" type="A">
                                            ...
                                            <li className="seat" style={{ marginLeft: '40px' }}>
